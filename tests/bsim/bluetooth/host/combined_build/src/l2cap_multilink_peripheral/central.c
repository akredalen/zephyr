...
LOG_MODULE_REGISTER(l2cap_mlp_central, LOG_LEVEL_DBG);
...
/* Read the comments on `entrypoint_multilink_peripheral_dut()` first. */
void entrypoint_multilink_peripheral_central(void)
{
	int err;
	...
	TEST_START("multiplink peripheral central");
	 ...
	 * termination happens.
	 */
	TEST_PASS("multiplink peripheral central");
