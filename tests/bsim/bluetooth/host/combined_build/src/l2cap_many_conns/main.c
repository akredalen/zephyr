...
/* Avoid test coexistence issues with conn_callbacks.
 *
 * This switch is also used to limit the scope of the data pull spy to the tests that need it.
 * An alternative to dynamic registration of bt_test_l2cap_data_pull_spy.
 * 
 * Set to true in local tests */
static bool local_test_run = false;
...
static struct test_ctx *get_ctx(struct bt_l2cap_chan *chan)
{
...
static struct test_ctx *alloc_test_context(void)
{
...
static const struct bst_test_instance test_def[] = {
	{
		.test_id = "l2cap/many_conns/peripheral",
		.test_descr = "Peripheral L2CAP LATENCY",
		.test_main_f = test_peripheral_main
	},
	{
		.test_id = "l2cap/many_conns/central",
...
struct bst_test_list *test_main_l2cap_many_conns_install(struct bst_test_list *tests)
{
