...
# Execute "${test_exe}" -s=${simulation_id} -d=0 -rs=420 -RealEncryption=1 \
#     -testid=l2cap/multilink_peripheral/dut
...
# Execute "${test_exe}" -s=${simulation_id} -d=1 -rs=169 -RealEncryption=1 \
#     -testid=l2cap/multilink_peripheral/central -delay_init -start_offset=1e3
# Execute "${test_exe}" -s=${simulation_id} -d=2 -rs=690 -RealEncryption=1 \
#     -testid=l2cap/multilink_peripheral/central -delay_init -start_offset=10e3
